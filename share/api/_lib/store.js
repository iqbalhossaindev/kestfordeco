...
const globalState = globalThis.__KFORD_SHARE_V3_STORE__ || {
  sessions: new Map(),
  queues: new Map(),
  nearby: new Map(),
  rates: new Map()
};
globalThis.__KFORD_SHARE_V3_STORE__ = globalState;
...
function sessionKey(code) {
  return `share:v3:session:${code}`;
}

function queueKey(code, role) {
  return `share:v3:queue:${code}:${role}`;
}

function nearbyKey(hash) {
  return `share:v3:nearby:${hash}`;
}

function rateKey(key) {
  return `share:v3:rate:${key}`;
}
